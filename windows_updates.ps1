...
$reportpath = "C:\Users\zabbix\update"

# Change $ZabbixInstallPath to wherever your Zabbix Agent is installed
$ZabbixInstallPath = "C:\Users\zabbix\bin"
$ZabbixConfFile = "C:\Users\zabbix\conf"
...
if ($countCritical -gt 0 -Or $countOptional -gt 2) {
	...
	$ReportFile = $reportpath + "\" + $hostname + " - Windows Update Report - "  + $TodayFile + ".txt"
	...
	If ($Result.Updates.Count -EQ 0) {
		...
		Add-Content $ReportFile "List of applicable updates for this computer"
		Add-Content $ReportFile "------------------------------------------------`r"
		...
		For ($Counter = 0; $Counter -LT $Result.Updates.Count; $Counter++) {
			...
			$DisplayCount = $Counter + 1
			Add-Content $ReportFile "`t $DisplayCount - Downloading update $ShowThis"
			$Downloader.Updates = $UpdateCollection
			$Track = $Downloader.Download()
			If (($Track.HResult -EQ 0) -AND ($Track.ResultCode -EQ 2)) {
				Add-Content $ReportFile "`t Download status: SUCCESS `r"
				#Add-content $ReportFile "`r"
			} else {
				Add-Content $ReportFile "`t Download status: FAILED With Error -- $Error `r"
				$Error.Clear()
				#Add-content $ReportFile "`r"
			}	
		}
		...
		For ($Counter = 0; $Counter -LT $UpdateCollection.Count; $Counter++) {
			...
			Try {
				...
				Add-Content $ReportFile "`t Update installation status: SUCCESS `r"
				#Add-content $ReportFile "`r"
			}
			Catch {
				[System.Exception]
				Add-Content $ReportFile "`t Update installation status: FAILED With Error -- $Error `r"
				$Error.Clear()
				#Add-content $ReportFile "`r"
			}	
		}
